import { ContractDocDownloadReqDto, ContractDocDraftListReqDto, ContractDocListReqDto, ContractDocUploadReqDto } from "../../1_inbound/requests/contract-doc-schema.request";
...
import { ContractDocEntity, PersistContractDocEntity } from "../entities/cotract-doc/contract-doc.entity";
...
export interface IContractDocService {
  ...
  getDraftContracts(dto: ContractDocDraftListReqDto): Promise<ContractDocViewEntity[]>;
  uploadContractDoc(dto: ContractDocUploadReqDto): Promise<PersistContractDocEntity>;
  downloadcontractDocs(dto: ContractDocDownloadReqDto): Promise<PersistContractDocEntity>;
}
...
export class ContractDocService extends BaseService implements IContractDocService {
  ...
  async getDraftContracts(dto: ContractDocDraftListReqDto): Promise<ContractDocViewEntity[]> {
    return this._unitOfWork.do(async (repos) => {
      const foundUser = await repos.user.findUserById(dto.userId);
  ...
  async uploadContractDoc(dto: ContractDocUploadReqDto): Promise<PersistContractDocEntity> {
    return this._unitOfWork.do(async (repos) => {
      const foundUser = await repos.user.findUserById(dto.userId);
  ...
  async downloadcontractDocs(dto: ContractDocDownloadReqDto): Promise<PersistContractDocEntity> {
    return this._unitOfWork.do(async (repos) => {
      const foundUser = await repos.user.findUserById(dto.userId);
